...
class NavMeshRaw:
    """Wraps an already-allocated dtNavMesh* without owning its lifetime.

    Used by TileCache to expose the inner navmesh to NavMeshQuery and Crowd
    without risk of double-free (the TileCacheHandle owns the dtNavMesh*).
    """

    def __init__(self, raw_ptr, lib):
        self._handle = raw_ptr
        self._lib = lib

    def close(self):
        pass  # lifetime is owned by TileCacheHandle, not us
